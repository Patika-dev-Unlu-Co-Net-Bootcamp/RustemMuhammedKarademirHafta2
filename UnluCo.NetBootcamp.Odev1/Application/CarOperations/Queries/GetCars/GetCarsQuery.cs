...
namespace UnluCo.NetBootcamp.Odev1.Application.CarOperations.Queries.GetCars
{
    public class GetCarsQuery
    {
        ...
        {
            var carList = _dbContext.Cars.Include(x => x.Brand).Where(x=>x.IsActive).OrderBy(x => x.CarId).ToList();//CarSystemDbContext altinda tanimli Cars listesinde carId ile eslesen ve IsActive degeri true olan kayit var mi diye kontrol eder.
